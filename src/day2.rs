...
impl Day2 {
    pub fn load(filename: &str) -> Day2 {
        ...
        for line in reader.lines() {
            ...
            if parts.len() == 2 {
                ...
                let mut command = Command { dir: Dir::FORWARD, dist: 0 };
                match parts[0] {
                    "forward" => { command.dir = Dir::FORWARD; }
                    "down" => { command.dir = Dir::DOWN; }
                    "up" => { command.dir = Dir::UP; }
                    _ => { panic!(); }
                }
    ...
    fn follow_course(&self, mut pos: (usize, usize)) -> (usize, usize) {
        for cmd in &self.commands {
            match &cmd.dir {
                Dir::FORWARD => { pos.0 = pos.0 + cmd.dist; } // increase fwd pos
                Dir::DOWN => { pos.1 = pos.1 + cmd.dist; }    // increase depth
                Dir::UP => { pos.1 = pos.1 - cmd.dist; }      // decrease depth
            }
        }
